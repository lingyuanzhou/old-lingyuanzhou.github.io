...
function multilevelLayout(G, Weight, tol) {	
	...
	if(nextLevelN < 2 ) {
		...
		/*var xtotal = Weight.matrix[0][0]+Weight.matrix[1][0];
		var x0 = G.spring*Weight.matrix[0][0]/xtotal;
		var x1 = G.spring*Weight.matrix[1][0]/xtotal;
		XVectors[0] = {};
		XVectors[0].pos = {x: centerX - x0, y: centerY};
		XVectors[0].disp = {x: 0, y: 0};
		XVectors[1] = {};
		XVectors[1].pos = {x: centerX + x1, y: centerY};
		XVectors[1].disp = {x: 0, y: 0};*/
...
function forceDirectedAlgorithm(G, XVectors, Weight, tol) {
	...
	/*while(!converged) {
		fdRepulsiveForce(G, XVectors, k2, Weight, R);
	...
	while(converged != 1) {
		...
		//var difference = 0;
		for(var i=0; i<length; i++) {
			fdRepulsiveForceS(i, G, XVectors, k2, Weight, R);
...
function fdRepulsiveForce(G, XVectors, k2, Weight) {
...
function fdRepulsiveForceS(index, G, XVectors, k2, Weight, R) {
